...
WEIGHT_MATRIX = np.array([[4**6, 4**5, 4**4, 4**3], [4**5, 4**4, 4**3, 4**2], [4**4, 4**3, 4**2, 4**1], [4**3, 4**2, 4**1, 4**0]])
...
class AI():
    ...
    def eval_board(self, board, n_empty): 
        ...
        empty_w = 1000 # Empty weight
        smoothness_w = 3 # Smootheness weight

        empty_u = n_empty * empty_w             # Empty utility 
        smooth_u = smoothness ** smoothness_w   # Smooth utility
        big_t_u = big_t                         # Big_t utility
        # arrangement_u = np.sum(WEIGHT_MATRIX @ grid)

        utility += big_t
        utility += empty_u
        utility += smooth_u
        # utility += arrangement_u
...
if __name__ == "__main__":
    print(np.max(np.array([[1,0],[0,2]]) @ np.array([[1,0],[0,2]])))